...
export async function retrieveUserInfo(element) {
  await CompasUserInfoService().getCompasUserInfo().then((response) => {
    ...
    element.dispatchEvent(newSetSessionTimeoutsEvent(parseInt(sessionWarning), parseInt(sessionExpires)));
  }).catch((reason) => {
