...
export function extractErrorMessage(doc) {
...
export function parseXml(textContent) {
  return Promise.resolve(new DOMParser().parseFromString(textContent, "application/xml"));
}
export function extractSclFromResponse(response) {
...
export function createLogEvent(element, reason) {
...
export function createNSDocLogEvent(element, fileName) {
  if (element) {
    element.dispatchEvent(newLogEvent({
      kind: "warning",
...
export function getWebsocketUri(settingsUrl) {
  if (settingsUrl.startsWith("http://") || settingsUrl.startsWith("https://")) {
    return settingsUrl.replace("http://", "ws://").replace("https://", "wss://");
  }
  return (document.location.protocol == "http:" ? "ws://" : "wss://") + document.location.hostname + ":" + getWebsocketPort() + settingsUrl;
}
