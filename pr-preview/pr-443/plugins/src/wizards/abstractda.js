...
function selectType(e, data, Val) {
  ...
  if (selectedBType !== "Enum")
    return;
  const enumVals = Array.from(data.querySelectorAll(`EnumType[id="${typeSelected}"] > EnumVal`)).map((enumval) => oscdHtml`<mwc-list-item
...
export function wizardContent(name, desc, bType, types, type, sAddr, valKind, valImport, Val, data) {
  return [
    ...
    oscdHtml`<wizard-select
      ...
      required
      fixedMenuPosition
      >${valKindEnum.map((valKindOption) => oscdHtml`<mwc-list-item value="${valKindOption}"
    ...
    oscdHtml`<wizard-select
      ...
      helper="${get("scl.Val")}"
      nullable
      >${Array.from(data.querySelectorAll(`EnumType > EnumVal[id="${type}"]`)).map((enumVal) => oscdHtml`<mwc-list-item value="${enumVal.textContent?.trim() ?? ""}"
            >${enumVal.textContent?.trim()}</mwc-list-item
          >`)}</wizard-select
    >`,
    oscdHtml`<wizard-textfield
