...
import { getValue, isPublic, newWizardEvent, } from '../../../_snowpack/pkg/@compas-oscd/open-scd/dist/foundation.js';
...
export function renderDa(fc, dchg, qchg, dupd) {
    return [
      ...
      required
      fixedMenuPosition
      >${functionalConstraintEnum.map(fcOption => oscdHtml `<mwc-list-item value="${fcOption}">${fcOption}</mwc-list-item>`)}</wizard-select
    >`,
