...
export function contentGseControlWizard(content) {
    ...
    ></wizard-textfield>`,
      ...
      nullable
      required
      >${['GOOSE', 'GSSE'].map(type => oscdHtml `<mwc-list-item value="${type}">${type}</mwc-list-item>`)}</wizard-select
    >`,
        oscdHtml `<wizard-textfield
      label="appID"
    ...
    ></wizard-checkbox>`,
      ...
      >${securityEnabledEnum.map(type => oscdHtml `<mwc-list-item value="${type}">${type}</mwc-list-item>`)}</wizard-select
    >`,
    ];
}
    ...
    return [
        {
          ...
          >${gseControls.map(gseControl => oscdHtml `<mwc-list-item twoline value="${identity(gseControl)}"
