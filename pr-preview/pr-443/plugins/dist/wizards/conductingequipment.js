...
import { crossProduct, getValue, isPublic, } from '../../../_snowpack/pkg/@compas-oscd/open-scd/dist/foundation.js';
...
function renderTypeSelector(option, type) {
    return option === 'create'
      ...
      >
        ${Object.keys(types).map(v => oscdHtml `<mwc-list-item value="${v}">${types[v]}</mwc-list-item>`)}
      </mwc-select>`
        : oscdHtml `<mwc-select
        label="type"
        helper="${get('conductingequipment.wizard.typeHelper')}"
