...
import { find, identity, pathParts, } from '../../../_snowpack/pkg/@compas-oscd/open-scd/dist/foundation.js';
        ...
        >
          <filtered-list
            ...
            >${reportItems
                    ...
                    .map(item => oscdHtml `<mwc-check-list-item
                    left
                    hasMeta
                    twoline
                    value="${item.identity}"
          ...
          ><filtered-list
            ...
            >${clientLns.map(ln => oscdHtml `<mwc-check-list-item twoline value="${identity(ln)}">
                  <span>${getElement(identity(ln))}</span>
                  <span slot="secondary">${getPath(identity(ln))}</span>
                </mwc-check-list-item>`)}
            <li divider role="separator"></li>
            ${serverLns.map(ln => oscdHtml `<mwc-check-list-item twoline value="${identity(ln)}">
                  <span>${getElement(identity(ln))}</span>
                  <span slot="secondary">${getPath(identity(ln))}</span>
                </mwc-check-list-item>`)}
            ${serverLn0s.map(ln0 => oscdHtml `<mwc-check-list-item twoline value="${identity(ln0)}">
                  <span>LLN0</span>
