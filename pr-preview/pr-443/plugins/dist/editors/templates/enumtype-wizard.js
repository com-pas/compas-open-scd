...
export function createEnumTypeWizard(parent, templates) {
    return [
        ...
        >
          ${Array.from(templates.querySelectorAll('EnumType')).map(e => oscdHtml `<mwc-list-item
                graphic="icon"
                hasMeta
...
export function eNumTypeEditWizard(eNumTypeIdentity, doc) {
    ...
    return [
        ...
        ></wizard-textfield>`,
          ...
          >${Array.from(enumtype.querySelectorAll('EnumVal')).map(enumval => oscdHtml `<mwc-list-item
                graphic="icon"
                hasMeta
                tabindex="0"
