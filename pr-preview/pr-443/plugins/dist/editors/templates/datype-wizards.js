...
import { find, getValue, identity, newSubWizardEvent, newWizardEvent, patterns, } from '../../../../_snowpack/pkg/@compas-oscd/open-scd/dist/foundation.js';
...
export function editDaTypeWizard(dATypeIdentity, doc) {
    ...
    return [
        ...
        >
          ${Array.from(datype.querySelectorAll('BDA')).map(bda => oscdHtml `<mwc-list-item twoline tabindex="0" value="${identity(bda)}"
...
export function createDATypeWizard(parent, templates) {
    return [
        ...
        >
          ${Array.from(templates.querySelectorAll('DAType')).map(datype => oscdHtml `<mwc-list-item
                graphic="icon"
                hasMeta
