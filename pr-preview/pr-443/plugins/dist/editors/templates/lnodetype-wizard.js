...
import { find, getValue, identity, isPublic, newSubWizardEvent, newWizardEvent, patterns, } from '../../../../_snowpack/pkg/@compas-oscd/open-scd/dist/foundation.js';
...
function dOWizard(options) {
    ...
    return [
        ...
        ></wizard-textfield>`,
          ...
          required
          helper="${get('scl.type')}"
          >${types.map(dataType => oscdHtml `<mwc-list-item
                value=${dataType.id}
    ...
    return [
        {
          ...
          ?nullable=${presCond !== 'M'}
          .maybeValue=${null}
          >${validDOTypes.map(doType => oscdHtml `<mwc-list-item value="${doType.getAttribute('id')}"
...
export function lNodeTypeWizard(lNodeTypeIdentity, doc) {
    ...
    return [
        ...
        ></wizard-textfield>`,
          ...
          >
            ${Array.from(lnodetype.querySelectorAll('DO')).map(doelement => oscdHtml `<mwc-list-item
                  twoline
                  tabindex="0"
