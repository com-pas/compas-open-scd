...
export function createDOTypeWizard(parent, templates) {
    return [
        ...
        >
          ${Array.from(templates.querySelectorAll('DOType')).map(datype => oscdHtml `<mwc-list-item
                graphic="icon"
                hasMeta
...
export function dOTypeWizard(dOTypeIdentity, doc) {
    ...
    return [
        ...
        ></wizard-textfield>`,
          ...
          >
            ${Array.from(dotype.querySelectorAll('SDO, DA')).map(daorsdo => oscdHtml `<mwc-list-item
                  twoline
                  tabindex="0"
